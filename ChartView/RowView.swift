...
open class RowView: UIView {
  ...
  open func setupColumns() {
    ...
    for columnNumber in 0..<numberOfColumns {
      ...
      constraints.append(
        cv.topAnchor.constraint(equalTo: topAnchor, constant: 0)
      )
      constraints.append(
        cv.bottomAnchor.constraint(equalTo: bottomAnchor, constant: 0)
      )
    ...
    if let last = columnViews.last {
      let constraint = last.rightAnchor.constraint(
        equalTo: rightAnchor,
        constant: 0
      )
