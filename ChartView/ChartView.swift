...
open class ChartView: UIView {
  ...
  public var configurationClosure: ((Int, RowView) -> Void)?
  ...
  fileprivate func checkAndAddEmpty() {
    if numberOfRows == 0, let text = emptyText {
      if emptyBottomConstraint == nil {
        emptyBottomConstraint = topAnchor.constraint(
          equalTo: bottomAnchor,
          constant: -emptyHeight
        )
  ...
  func setupRows() {
    ...
    for rowNumber in 0..<numberOfRows {
      ...
      ch.left = rv.leftAnchor.constraint(
        equalTo: leftAnchor,
        constant: rowSpacing
      )
      ch.right = rv.rightAnchor.constraint(
        equalTo: rightAnchor,
        constant: -rowSpacing
      )
    ...
    if let last = rowViews.last {
      ...
      bottomConstraint = last.bottomAnchor.constraint(
        equalTo: bottomAnchor,
        constant: -rowSpacing
      )
    ...
    NSLayoutConstraint.activate(
      constraintHolders.flatMap { $0.all }
    ...
    if pgr.state == .ended || pgr.state == .cancelled || pgr.state == .failed {
      if translation.x < 0 && movement > (pgr.view!.frame.width / 3) {
        NotificationCenter.default.post(
          name: Notification.Name(rawValue: "chartViewWillDelete"),
          object: self
        )
...
extension ChartView {
  func deleteRowView(_ rowView: RowView, velocity: CGFloat) {
    ...
    guard chartViewDataSource.numberOfRows + 1 == rowViews.count else {
      fatalError("You needed to remove an element from the dataSource")
    }
    ...
    if rowViews.count == 0 {
      ...
      self.bottomConstraint = self.bottomAnchor.constraint(
      ...
      originalPriority = singleConstraint.priority
      (removingCH.all + [singleConstraint]).forEach { $0.isActive = false }
      belowCH.top = belowCH.rowView.topAnchor.constraint(
        equalTo: self.topAnchor,
      ...
      originalPriority = singleConstraint.priority
      (removingCH.all + [singleConstraint]).forEach { $0.isActive = false }
      self.bottomConstraint = aboveCH.rowView.bottomAnchor.constraint(
      ...
      originalPriority = singleConstraint.priority
      (removingCH.all + [singleConstraint]).forEach { $0.isActive = false }
      belowCH.top = aboveCH.rowView.bottomAnchor.constraint(
        equalTo: belowCH.rowView.topAnchor,
        constant: -self.rowSpacing
      )
      belowCH.top.priority = UILayoutPriority(rawValue: 100)
      belowCH.top.isActive = true
    ...
    UIView.animate(
      ...
      animations: {
        rv.alpha = 0
        rv.transform = CGAffineTransform(
          translationX: -(30 + rv.frame.width),
          y: 0
        )
      },
      completion: { _ in
        ...
        singleConstraint = singleConstraint.firstAnchor.constraint(
          equalTo: singleConstraint.secondAnchor!,
          constant: singleConstraint.constant
        )
    ...
    NotificationCenter.default.post(
      name: Notification.Name(rawValue: "chartViewDidDelete"),
      object: self
    )
  }
