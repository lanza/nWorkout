...
class StatisticsChartsTVC: BaseTVC {
  ...
  override func viewDidLoad() {
    ...
    let bestSetDataEntries = bestSetDataPoints.map {
      ChartDataEntry(x: $0.timeInterval, y: $0.weight)
    }
    let bestSetDataSet = CustomLineChartDataSet(
      entries: bestSetDataEntries,
      label: "Best Set Progression"
    )
    ...
    let prDataEntries = prDataPoints.map {
      ChartDataEntry(x: $0.timeInterval, y: $0.weight)
    }
    let prDataSet = CustomLineChartDataSet(
      entries: prDataEntries,
      label: "Personal Record Progression"
    )
    let prLineChartData = LineChartData(dataSet: prDataSet)

    let sectionModel = ChartSectionModel(
      chartData: [bestSetLineChartData, prLineChartData]
    )
